...
Every candidate receives a dedicated fresh repository plus private home, temp, cache, and config roots. Configured workspace-root names are logical namespace inputs: candidate roots resolve to separate opaque per-attempt OS-temporary locations, while the private registry resolves to a source-bound host state location outside the source repository. Candidate environments discard inherited working-directory, controller, registry, task, Git-routing, and credential handles. The snapshot has a private `.git` database with no remotes, alternates, worktree links, future refs/objects, controller route, hidden graders, reference solutions, sibling paths, or registry path. The receipt truthfully declares a trusted-host boundary rather than claiming a container or hostile-host filesystem sandbox.
